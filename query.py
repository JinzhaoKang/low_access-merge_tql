...
#         access[i] = (query[1])
#     # sum_response = np.sum(ans_array * modified_w.reshape(-1, 1), axis=1)
...
def query(w1, nodes_array):
    ...
    # partition w and do a query
    access = np.zeros(m)
    ans_array, access[0] = nodes_array[0].query(modified_w[width*0:width*(0+1)])

    for i in range(1,m):
        query = nodes_array[i].query(modified_w[width*i:width*(i+1)])
        ans_array = np.vstack((query[0]))
        access[i] = (query[1])
